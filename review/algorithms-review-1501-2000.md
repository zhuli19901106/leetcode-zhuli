...
题意：162的变体。给定一个m x n矩阵，如果一个值比<b>上下左右</b>四个值都大，则称为<b>峰值</b>。我们假定边界外是<b>负无穷</b>。请找出任意一个峰值的位置`(i,j)`。要求在低于`O(n^2)`的时间完成。

难度：medium

解法：考虑了很多做法，都不太行。比如横、竖两个维度进行<b>二分</b>。最后还是按照<b>题目的提示</b>，考虑`O(mlogn)`或者`O(nlogm)`。这个复杂度的形式，其实是<b>暗示你</b>应该一个维度<b>枚举</b>，一个维度<b>二分</b>。比如我们<b>按行二分，按列枚举</b>。那么在某i行，我用`O(n)`代价找出<b>最大值</b>`a[i][max_j]`。这时我比较`(a[i-1][max_j],a[i][max_j],a[i+1][max_j])`三者的大小。如果是大大，则应该向上走；如果是小小，则应该向下走；如果是小大，则`a[i][max_j]`就是峰值；如果是大小，则上下走都行。这个思路有点难想，但说出来之后，会发现也挺直观的。需要一点<b>形象思维</b>，想象出一个<b>高低不平的地形图</b>。对medium难度来说，这题算很难了。

<hr>

1903 Largest Odd Number in String

题意：给定数字串s，请从中找出一个子串，使其代表能得到的<b>最大奇数</b>。

难度：easy

解法：奇数，那我们只关心最后一位是奇数即可。可以<b>从后往前</b>找出<b>第一个奇数</b>，位置是i。那么`s[0:i+1]`就是答案。总代价`O(n)`。

<hr>

1905 Count Sub Islands

题意：给定两个m x n的01矩阵g1、g2，0水1陆。上下左右相连的1格子，代表一个小岛。我们先求出g1中的各个小岛，然后求g2中的小岛。对于g2中的小岛，如果一个小岛的所有格子，都对应属于<b>g1中的同一小岛</b>，也就是<b>被包含</b>，我们称g2这个小岛为<b>“子岛”</b>。

难度：medium

解法：可以用<b>并查集</b>的方式把属于<b>同一小岛</b>的格子归到一类。先对g1执行一次搜索，求出<b>不同的小岛</b>，进行编号（或者任何有效的区分方式）。再对g2执行一次搜索，求出不同的小岛。然后对g2中的每个小岛，对每个格子`(i,j)`，<b>针对g1</b>求出`findRoot(dj, (i,j))`是否都等于同一个根节点，也就是属于<b>同一个小岛</b>。如果属于，那就是<b>子岛</b>；如果发现多个根节点，则属于不同小岛，就不是子岛。总体思路不算难，但要做的事情很多，<b>比较累人的一题</b>。写法适当的话，代码也能做到比较短。总代价`O(mn)`。

<hr>

1909 Remove One Element to Make the Array Strictly Increasing

题意：给定数组nums，请判断能否删除<b>至多一个元素</b>，使得数组严格递增。
...
题意：给定字符串s和t，不断从s中找出<b>最左边的一个t</b>并删除。请返回删除后剩余结果。

难度：medium

解法：按照给定数据量，不难想出一个`O(ns*nt)`的解法。用一个栈，不断把s[i]入栈。如果栈顶的<b>nt个元素</b>恰好构成一个<b>t串</b>，则<b>删除</b>。不过这个做法还不如纯粹的<b>暴力匹配</b>，直接`s.find(t)`就完事了。两者的复杂度是一样的。总代价`O(ns*nt)`。

<hr>

1911 Maximum Alternating Subsequence Sum

题意：给定数组nums，我们定义交替和为`sum(nums[0::2])-sum(nums[1::2])`，也就是<b>偶数位的和-奇数位的和</b>。现在请求出nums的任意<b>子序列</b>的最大交替和。

难度：medium

解法：子序列的最大交替和。想了下，应该是<b>DP思路</b>。这个求和方式是`a[0]-a[1]+a[2]-...`，那么根据奇偶性，我们知道应该<b>加还是减</b>。我们定义dp[i][j]为<b>序列长度i</b>，以`nums[j]`结尾的最大交替和。那么下一步的递推关系，就根据<b>i的奇偶性</b>来定。这个思路的复杂度明显太高，`O(n^3)`，不可接受。DP的思路不行。题目暗示的复杂度是`O(n)`，一定有更巧妙的办法。其实<b>DP思路是对的</b>，但做法得换一个。我用ve、vo表示偶长度、奇长度的<b>目前最优解</b>。那么`ve1=max(ve,vo+x);vo1=max(0,max(vo,ve-x));`。<b>奇偶交替</b>递推，而且带上一个`max(0,...)`防止出现负值。显然，这个<b>思路有难度</b>，不一定想得到。<b>一旦想明白了</b>，就超级简单，总代价`O(n)`。

<hr>

1913 Maximum Product Difference Between Two Pairs

题意：给定数组nums，请从其中找出四个不同元素abcd，使得`ab-cd`最大。

难度：easy

解法：找出<b>最小、最大</b>的<b>各两个</b>元素即可。

<hr>

1915 Number of Wonderful Substrings

题意：一个字符串，如果至多一种字符出现次数是奇数，称为<b>好字符串</b>。给定字符串s，求出s的<b>子串</b>中，好字符串的个数。

难度：medium

解法：子串个数，第一念头是<b>滑动窗口</b>。但似乎想了挺久，觉得<b>想不通</b>。这个应该不对，因为滑动窗口是具有<b>单调性</b>的。一旦越过了界限，就<b>yes变no</b>，或者<b>no变yes</b>。而<b>奇偶性</b>是可以<b>交替变化</b>的，不存在<b>唯一的界限</b>。最后，我考虑既然是26个字符，奇偶性，那我就用<b>26位二进制数</b>，来表示奇偶。然后`countOne()`统计二进制1的个数好了。有了这个思路，剩下就是计算<b>前缀bit mask</b>。如此一来，把这些bm放入哈希进行计数。比如`countOne(bm[i]^bm[j])<=1`，这代表什么？这代表`s[i+1:j+1]`这个子串是个<b>好串</b>。想明白这个，这题就搞定了。总代价`O(26*n)=O(n)`。这题对我来说，比很多hard都难，主要在于<b>难想</b>。

<hr>

1920 Build Array from Permutation

题意：给定`0~n-1`的排列nums，请执行一次<b>置换</b>，即`nums[nums[i]]`，求出变换后的结果。
...
题意：有n个怪物朝你走来，距离、速度分别为`dist[i]`、`speed[i]`。已知你有一个大炮，发射后需要等待1s才能充能。0时刻大炮已经充能，如果有怪物到达你的位置，则任务失败。请求出你<b>最多能消灭</b>的怪物数量。

难度：medium

解法：这题主要在于理解这个数学模型，你发射后，需要等1s。那么你应该怎么办？当然是在0、1、2、...<b>时刻</b>马上开炮，这样你没有<b>浪费任何时间</b>。因此，求出每个`dist[i]/speed[i]`，也就是到达时间，<b>按时间排序</b>。如果第i个怪物在<b>i秒之前到达</b>，你就g了。否则，你就可以击败它。总代价`O(nlogn)`。

<hr>

1925 Count Square Sum Triples

题意：给定n，请找出[1,n]范围内的三元组(a,b,c)，使得`a^2+b^2=c^2`。返回个数。

难度：easy

解法：注意顺序，345和435算两个。这就是<b>勾股数</b>，勾股定理。勾股数并不多，在给定范围内，直接枚举一下都行。

<hr>

1929 Concatenation of Array

题意：给定数组nums，请返回`nums+nums`。
...
题意：给定字符串s，请求出长度为3的<b>回文子序列</b>的个数。

难度：medium

解法：长度为3的回文串，那就是aba形式的子串。对于s[i]位置，我考虑前缀`s[0:i]`和后缀`s[i+1:n]`。对这两个串，做<b>哈希计数</b>，得到mm1、mm2。那么对于同一字符c，`mm1[c]*mm2[c]`就是<b>左右各取</b>一个c字符的个数。因此，对于每个s[i]位置，我求出`sum(mm1[c]*mm2[c])`。再对这些sum求和，就是最终结果。至于mm1、mm2的更新，在<b>i移动的过程中</b>，单次更新的代价是`O(1)`，总代价`O(n)`。

<hr>

1933 Check if String Is Decomposable Into Value-Equal Substrings

题意：给定字符串，请判断能否将其划分为多个<b>单值子串</b>，其中一个长度为2，其他长度为3。

难度：easy

解法：付费题。水题，判断<b>连续单值串</b>的长度，如果<b>恰好一个</b>`%3==2`，则满足要求。

<hr>

1935 Maximum Number of Words You Can Type

题意：给定一个英文句子，和键盘上<b>坏掉的</b>字母键。请求出句子中，你可以<b>完整打出</b>的单词个数。

难度：easy

解法：水题。先分词，然后判断是否包含坏掉的字母。

<hr>

1940 Longest Common Subsequence Between Sorted Arrays

题意：给定n个严格递增的数组，请求出n个数组的<b>最长公共子序列</b>。返回子序列。

难度：medium

解法：付费题。因为<b>数组有序</b>，可以采用n个有序列表归并的思路。n个指针，都从0位置出发。采用一个<b>最小堆</b>，每次都找最小的值。如果发现当前最小值x，恰好等于<b>所有表头的值</b>，那说明x是一个<b>公共值</b>。这样每次通过`O(logn)`代价可以<b>更新最小堆</b>。总代价`O(sum(len(a[i]))*log(n))`。当然，n个数组归并，也可以按照2个数组归并，<b>执行`n-1`次</b>。这样稍微慢一点，但代码更简单。

<hr>

1941 Check if All Characters Have Equal Number of Occurrences

题意：给定字符串s，请判断所有字符的<b>频率</b>是否相同。
...
题意：有n个人，编号`0~n-1`。有无限个座位，编号也从0开始。有人来时，都会选当前空余且编号最小的座位。有人走时，则空出之前占用的座位。现在给定n个人的`[到来,离开]`时间，请求出编号为`target`的人坐的座位。

难度：medium

解法：之前考虑用<b>区间</b>处理，后来想了想发现<b>太麻烦了</b>。还是把n个座位都视为<b>独立个体</b>吧。用一个<b>SortedSet</b>来记录空闲座位。这样能以`O(logn)`代价处理座位的变化，以及找到编号最小的空闲座位。

<hr>

1943 Describe the Painting

题意：给定一些区间，用`[x,y,c]`表示。意思是从x到y颜色为c。现在将这些区间进行处理，同一位置如果有多个颜色，则合并作为一种混色`[c1,c2,...]`。请将这些区间进行合并、分拆，得到不想交的混色区间列表。对每种混色，输出格式为`[x,y,sum([c1,c2,...])]`，也就是对颜色值求和。本题的区间是<b>左闭右开</b>区间`[x,y)`。

难度：medium

解法：这题的题目描述实在太糟糕，<b>冗长而费解</b>。其实你注意一个例子就够了，`[5,7]`和`[1,11]`是两种不同颜色，虽然求和都等于12。对于区间处理，一种常用的做法是拆分`[x,y)`为`[0,x)`和`[0,y)`两个，然后执行`mm[x]-=c;mm[y]+=c;`两步。然后按照<b>key从大到小</b>，执行<b>后缀累加</b>。当然这个做法<b>挺巧妙的</b>。同样的思路，你用其他方式也能写，复杂度可以差别很大。这个后缀累加的做法，要基于排序或者SortedDict<b>有序结构</b>。因此，总代价`O(nlogn)`。我的第一版代码就超时了，思路虽然接近，但不够优化。

<hr>

1944 Number of Visible People in a Queue

题意：有n个人，每个人身高是`heights[i]`，身高各不相同。现在从左到右排队，对于第i人，如果第`i+1~j-1`人的身高都小于<b>i人和j人</b>，则<b>第i人可以看到第j人</b>。现在请求出每个人<b>右边可以看到</b>的人的<b>个数</b>。

难度：hard

解法：挺有意思，又是个<b>比大小、单调性</b>之类的问题。是不是很容易想到<b>单调栈、单调队列</b>的思路？我们先看个例子`[10,6,8,5,11,9]`。10能看见6、8、11，看不见5、9。那么10在11之后，就什么都看不见了，因为`10<11`，<b>挡住了</b>。所以，我们用<b>栈</b>，当`x<y`时，x就可以出栈。这个栈的变化如下`[]->[10]->[10,6]->[10,6,8]->[10,8]->[10,8,5]->[10,8,5,11]->[10,8,11]->[10,11]->[11]->[11,9]`，想一想每一步时，<b>谁看见了谁</b>？注意，不要采用<b>每次+1</b>的方式去模拟整个变化，这样效率会降为`O(n^2)`。考虑每次<b>持续出栈</b>的次数，<b>一次性加起来</b>。这样，总代价才能保持`O(n)`。这题的代码非常简短，主要<b>难在思路</b>。

<hr>

1945 Sum of Digits of String After Convert

题意：给定字符串s，用`a-z`代表`1~26`，将其转换为数字串。然后对其进行k次变换，每次都把s变为s的<b>数字和</b>。
...
题意：有n个问题，让m个学生和m个导师分别回答这些问题。每个人的答案是一个<b>长度n的01数组</b>，分别用`students[i]`和`mentors[i]`表示。现在要给导师、学生找出一个一一对应的<b>匹配方式</b>，使得<b>总匹配度</b>最高。两个人的匹配度等于两人对n个问题，<b>回答相同</b>的个数。

难度：medium

解法：<b>m、n都非常小</b>，第一念头就是<b>位向量、位操作</b>。其实看到01数组、回答相同的个数，就很容易想到<b>异或</b>操作了（这个是<b>同或</b>）。那么我们把`students[i]、mentors[i]`都转成n位二进制数。然后针对所有可能的<b>排列</b>，直接<b>暴力搜索</b>就行了。这题其实是一个<b>二部图最大匹配</b>问题，直接暴力搜索的做法，复杂度是`O(m!)`，加上处理n长度位向量的代价，总代价`O(n*m!)`。

<hr>

1952 Three Divisors

题意：给定整数n，请判断n是否只有3个约数。
...
题意：有一个二维平面，在`(i,j)`位置有`|i|+|j|`个苹果。你需要以`(0,0)`为中心，围出一个正方形篱笆。请问周长至少多少，你才能采到至少`neededApples`个苹果。篱笆<b>边界上</b>的苹果可以采集。

难度：medium

解法：可以用草稿纸算一下，找找<b>数学规律</b>。然后写出一个以n为参数的方程。我用边长的一半作为n，则苹果个数是`(n+1)*2n*(2n+1)`。然后用<b>二分搜索</b>找出n即可。总代价`O(logn)`。

<hr>

1957 Delete Characters to Make Fancy String

题意：给定字符串s，请删除最少的字符，使得不存在<b>连续三个字符相同</b>。返回删除后的结果。

难度：easy

解法：统计连续个数，如果`>=3`，则删到2个为止。总代价`O(n)`。

<hr>

1961 Check If String Is a Prefix of Array

题意：给定字符串s和一个词表words。请判断是否存在一个words的前缀`words[0:i+1]`，使得s等于其中的单词<b>连接起来</b>。

难度：easy

解法：题意很明确。从`pref=''`开始，逐个`pref+=words[i]`，看这个过程中有没有等于s的结果。总代价`O(n)`。

<hr>

1962 Remove Stones to Minimize the Total

题意：有n堆石头，每堆有`piles[i]`个。每次你可以选一堆，拿走`floor(piles[i]/2)`个。经过k次操作后，请求出<b>剩余所有石头个数</b>的最小值。

难度：medium

解法：既然希望剩余的sum最小，那我们应该按照<b>贪心原则</b>，尽可能多拿，拿k次。这就很容易想了，用一个<b>最大堆</b>。每次都取出最大值x，拿走`floor(x/2)`，把`x-floor(x/2)`放回堆中。总代价`O(klogn)`。

<hr>

1963 Minimum Number of Swaps to Make the String Balanced

题意：给定一个括号串s，有n/2个左括号、右括号。允许你交换任意两个位置，请问<b>至少</b>多少次交换，能使得s变成一个<b>匹配的括号串</b>？

难度：medium

解法：首先，“()”的个数相等，那么不会出现多余的“(”。如果不匹配，必然是在前面出现了<b>多余的“)”</b>。那其实我只统计多余的右括号个数即可。但要注意，交换次数不等于求和，应该等于`(max(多余的右括号个数)+1)/2`，因为1次交换修正了2个不平衡的括号，是<b>2倍关系</b>。总代价`O(n)`。这题不算难，但想清楚这个关系还是有点挑战性。

<hr>

1964 Find the Longest Valid Obstacle Course at Each Position

题意：给定数组nums，对每个`nums[i]`，请求出包含`nums[i]`的最长递增子序列的长度，要求<b>严格递增</b>。

难度：hard

解法：原题目的obstacles之类的应用场景就不说了，翻译过来就是<b>最长递增子序列</b>（LIS）。在给定数据量下，`O(n^2)`肯定不行，需要用二分搜索进行加速，写一个`O(nlogn)`的解法。这个属于教科书级的经典做法，细节就不说了。关键在于，<b>必须包含`nums[i]`</b>，这个怎么办？其实很简单，<b>什么都不用做</b>。因为LIS的`O(nlogn)`解法，本身就是针对每个`nums[i]`，都考虑应该把它插入到<b>哪个位置</b>，通过二分快速找到那个位置。既然我<b>本来就要用</b>`nums[i]`，也就不用额外动脑子了。所以，这题是<b>原汁原味的LIS问题</b>，连半个字都没改。总代价`O(nlogn)`。

<hr>

1967 Number of Strings That Appear as Substrings in Word

题意：给定字符串word和一些模式串patterns。请判断有多少个模式在word中出现了。

难度：easy

解法：就是<b>多模式匹配</b>，作为easy难度，逐个匹配就行了。

<hr>

1968 Array With Elements Not Equal to Average of Neighbors

题意：给定数组nums，请<b>重排顺序</b>，使得对于任意i，都满足`2*nums[i]!=nums[i-1]+nums[i+1]`。返回任意的有效排列。

难度：medium

解法：如果我们一个个找位置插入，那总代价就是`O(n^2)`，太慢了。尝试想了一些奇奇怪怪的思路，比如递归生成、穿插生成之类的，都不靠谱。最后想了想，还是先<b>排序</b>，然后在<b>尾部</b>尝试插入。排序代价当然是`O(nlogn)`，试探插入的总代价理论上是`O(n^2)`，实际运行非常接近`O(n)`。因此，这个解法的实际效率不错。总代价应该算`O(nlogn)`。

<hr>

1971 Find if Path Exists in Graph

题意：给定无向图，请判断从src到dst是否存在路径。

难度：easy

解法：就是很标准的<b>搜索</b>。<b>BFS或者DFS</b>都行，总代价`O(E)`。

<hr>

1973 Count Nodes Equal to Sum of Descendants

题意：给定二叉树，请统计节点值等于<b>子树所有节点</b>值之和的个数。

难度：medium

解法：付费题。后序遍历，求和比较即可。总代价`O(n)`。

<hr>

1974 Minimum Time to Type Word Using Special Typewriter

题意：`a-z`排成<b>一圈</b>，初始你在a位置。如果<b>每1s</b>，你可以选择左右移动一格，或者输出当前字符。请问输出`word`表示的单词，至少需要多少秒？

难度：easy

解法：从`cur='a'`开始，计算和<b>下一个字符</b>的距离，再`+1`用于<b>输出字符</b>。累加起来就是<b>总时间</b>。总代价`O(n)`。

<hr>

1975 Maximum Matrix Sum

题意：给定一个n x n方阵，每次你可以任选<b>两个相邻元元素</b>，都`*-1`。经过任意次操作后，请求出得到矩阵的<b>元素和的最大值</b>。

难度：medium

解法：既然是元素和的最大值，我们当然希望每个元素都是正的。那么比如`a[i][j]`是负数，我可以通过<b>多次操作</b>，把这个变换移动到任何`a[i'][j']`位置。那也就是说，不论这个负号在哪儿，我都有办法把它们<b>聚拢到一起</b>。因此，我其实只关心<b>负数</b>有<b>奇数个还是偶数个</b>。如果偶数个，就全变正值；如果奇数个，我就取`min(abs(a[i][j]))`让它为负。鉴定为<b>坑爹智力题</b>，总代价`O(n^2)`。

<hr>

1979 Find Greatest Common Divisor of Array

题意：给定数组nums，求出`gcd(min(nums),max(nums))`。
...
题意：给定n个长度n的01串nums，请求出一个长度n的01串s，使得<b>s不在nums中</b>。

难度：medium

解法：n位01串总共有`2^n`个。这么多候选，随便找个不存在的就行。计算过程可以用位操作，稍微提高点效率。总代价`O(n^2)`。

<hr>

1984 Minimum Difference Between Highest and Lowest of K Scores

题意：给定数组nums，请从中选出k个元素，使得max-min的值最小。

难度：easy

解法：很显然，对nums进行<b>排序</b>，然后判断所有<b>k窗口</b>的`max-min`，找出<b>最小值</b>就是答案。总代价`O(nlogn)`。

<hr>

1991 Find the Middle Index in Array

题意：给定数组nums，请找出<b>最小的位置i</b>，使得`sum(nums[0:i])==sum(nums[i+1:n])`，也就是`nums[i]`左右两边的子数组，求和相等。如果不存在，则返回-1。

难度：easy

解法：保持一个<b>前缀和、后缀和</b>即可，随着i前进，不断更新两个和的值，<b>判断相等</b>。总代价`O(n)`。

<hr>

1992 Find All Groups of Farmland

题意：给定m x n的01矩阵，0表示森林，1表示农田。请找出所有矩形的1区域，并返回左上右下坐标，按`[r1,c1,r2,c2]`格式返回。

难度：medium

解法：题目保证这个区域一定是矩形的，那搜索必然从左上角开始搜了。搜索用标准的<b>BFS洪泛</b>方式就行。这题的解决方式还有很多种，因为矩形区域实在是<b>太整齐了</b>。总代价`O(mn)`。

<hr>

1995 Count Special Quadruplets

题意：给定数组nums，请求出其中满足`a<b<c<d,nums[a]+nums[b]+nums[c]==nums[d]`的四元组个数。

难度：easy

解法：数据量很小，随便怎么枚举都行。想了几种不同的方法，总时间都是`O(n^3)`，空间则有大有小。还是用最直接的办法好了，枚举abc，用哈希表记录d。这样的话，总时间`O(n^3)`，总空间`O(n)`。

<hr>

2000 Reverse Prefix of Word

题意：给定字符串s和字符c，请找出c出现的第一个位置i，并反转`s[0:i+1]`。如果没有出现，则什么都不做。
